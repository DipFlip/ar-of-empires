...
export class Tracker {
 constructor(video,manifest,onFrame,onError){
  ...
  this.canvas=document.createElement('canvas');this.ctx=this.canvas.getContext('2d',{willReadFrequently:true,alpha:false});this.busy=false;this.running=false;this.h=null;this.scanTimes=[];this.misses=0;this.metrics=null;this.frameCallback=null;this.runId=0;this.timingSamples=[];this.captureSurfaces=new Map();this.grayBuffers=new Map();this.lastDetection=-Infinity;this.trackedPoints=0;this.flowReady=false;
 }
 async init(){
  if(this.ready)return;
  const worker=this.worker=new Worker(new URL('./tracking-worker.js',import.meta.url),{type:'module'});
  this.worker.postMessage({type:'configure',markers:[...this.markers.values()]});
 ...
 receive(data){
  ...
  const board=data.board===undefined?estimateBoard(data.tags,this.markers):data.board;
  if(Number.isFinite(data.lastDetection))this.lastDetection=data.lastDetection;
  this.trackedPoints=data.trackedPoints||0;this.flowReady=data.flowReady===true;
  this.misses=board?0:this.misses+1;
  this.scanTimes.push(received);if(this.scanTimes.length>20)this.scanTimes.shift();
  this.metrics={...data.timings,returnMs:Number.isFinite(data.sentAt)?Math.max(0,performance.timeOrigin+received-data.sentAt):undefined,fitMs:(data.timings?.boardFitMs||0)+performance.now()-fitStart,latencyMs:received-data.timestamp,
   hz:this.scanTimes.length>1?(this.scanTimes.length-1)*1000/(received-this.scanTimes[0]):0,
   width:data.width,height:data.height,mode:data.mode||'detect',trackedPoints:this.trackedPoints,detected:data.detected??true,sampleTime:data.timestamp};
 ...
 scan(source=this.video){
  ...
  if(!this.captureSurfaces.has(key)){
   const canvas=this.captureSurfaces.size?document.createElement('canvas'):this.canvas;
   this.captureSurfaces.set(key,{canvas,ctx:canvas.getContext('2d',{willReadFrequently:true,alpha:false})});
  }
  ({canvas:this.canvas,ctx:this.ctx}=this.captureSurfaces.get(key));
  ...
  const bytes=this.canvas.width*this.canvas.height;
  const recycled=this.grayBuffers.get(bytes);this.grayBuffers.delete(bytes);
  const gray=new Uint8Array(recycled||new ArrayBuffer(bytes));
  ...
  this.busy=true;this.worker.postMessage({gray:gray.buffer,width:this.canvas.width,height:this.canvas.height,timestamp,prepMs,timings,forceDetect,postedAt:performance.timeOrigin+performance.now()},[gray.buffer]);
 }
 complete(data){
  if(data.gray)this.grayBuffers.set(data.gray.byteLength,data.gray);
  this.busy=false;
 ...
 start(){this.cancelFrame();this.runId++;this.running=true;this.h=null;this.lastSeen=0;this.scanTimes=[];this.misses=0;this.metrics=null;this.timingSamples=[];this.lastScanAt=undefined;this.lastCompletedAt=undefined;this.pendingPresentation=null;this.lastDetection=-Infinity;this.trackedPoints=0;this.flowReady=false;this.schedule();}
 stop(){this.captureSurfaces.clear();this.grayBuffers.clear();this.pendingPresentation=null;this.running=false;this.runId++;this.cancelFrame();this.h=null;this.busy=false;this.worker?.terminate();this.worker=null;this.ready=false;}
}
